...
def transaction_json_in_python(json_path: str = "data/operations.json") -> list | list[dict]:
    ...
    try:
        logger.info(f'transaction_json_in_python попытка открытия файла транзакций')
        with open(json_path, encoding="utf-8") as data_file:
            transaction_data = json.load(data_file)
            if not isinstance(transaction_data, list):
                logger.error(f'transaction_json_in_python данные не список, отдаем пустой список')
                transaction_data = []
        ...
        transaction_data = []
    logger.info(f'transaction_json_in_python чтение файла транзакций завершено')
...
def transaction_amount(transaction: dict) -> float:
    ...
    """
    logger.info(f'Вызов transaction_amount)')
    if transaction.get("operationAmount").get("currency").get("code") and transaction.get("operationAmount").get(
        ...
        logger.info(f'transaction_amount успешное выполнение функции)')
        return result_value
    else:
        logger.error(f'transaction_amount ошибка выполнения, сумма транзакций будет равна 0)')
        return 0
