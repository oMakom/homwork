...
def get_mask_card_number(number_card: int | str) -> str:
    ...
    for num in str(without_spaces_number_card):
        if num.isalpha():
            logger.error(f'get_mask_card_number ошибка "Буквенный ввод недопустим". Завершение функции')
        ...
        if counter % 4 == 0 and counter < len(str(without_spaces_number_card)):
            mask_card += " "
        counter += 1
    logger.info(f'get_mask_card_number завершение обработки номера карты')
    return mask_card
...
def get_mask_account(account_number: int | str) -> str:
    ...
    **XXXX
    ...
    for num in str(without_spaces_account_number):
        if num.isalpha():
            logger.error(f'get_mask_account ошибка "Буквенный ввод недопустим". Завершение функции')
            return "Буквенный ввод недопустим"
        if counter > len(str(without_spaces_account_number)) - 4:
            ...
            mask_account += "*"
        counter += 1
    logger.info(f'get_mask_account завершение обработки номера счета')
    return mask_account
